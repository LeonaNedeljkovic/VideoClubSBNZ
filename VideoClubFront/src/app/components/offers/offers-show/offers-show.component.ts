...
export class OffersShowComponent implements OnInit {
  ...
  getOffersForUser() {
    this.offerService
      .getOffersForUser()
      .subscribe((customOffers: OfferDto[]) => {
  ...
  purchase(offerId: number) {
    this.purchaseService
      .createPurchase(offerId.toString())
      .subscribe((data) => {
        if (localStorage.getItem("messagePurchase") != undefined) {
          var message2 = JSON.parse(localStorage.getItem("messagePurchase"));
          var messages: Message = {
            header: message2["message"],
          ...
          var message: Message = {
            header: "Purchase done!",
            message: "You have bought minutes successfully!",
            color: "green",
          };
