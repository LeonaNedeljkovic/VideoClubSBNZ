  ...
  createOffer(){
    if(this.offer.minutes > 0 && this.offer.price > 0){
      this.offerService.createOffer(this.offer).subscribe(data => {
        if(data.minutes==this.offer.minutes && data.price==this.offer.price){
          var message : Message = {header:"Offer created!", message:"You have created offer successfully!", color:"green"};
