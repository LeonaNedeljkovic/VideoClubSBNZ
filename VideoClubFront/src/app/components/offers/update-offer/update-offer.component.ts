...
export class UpdateOfferComponent implements OnInit {
  ...
  updateOffer() {
    if (this.offer.price > 0 && this.offer.minutes > 0) {
      this.offerService
        .updateOffer(this.offer.id, this.offer)
        .subscribe((data) => {
          if ((this.offer.minutes = data.minutes)) {
            var message: Message = {
              header: "Offer updated!",
              message: "You have updated offer successfully!",
              color: "green",
            };
            ...
            var message: Message = {
              header: "Update not successful!",
              message: "You have not updated offer successfully!",
              color: "green",
            };
            localStorage.setItem("message", JSON.stringify(message));
            const modalRef = this.modalService.open(MessageComponent);
      ...
      var message: Message = {
        header: "Error!",
        message: "Price and minutes must be greater than 0!",
        color: "green",
      };
      localStorage.setItem("message", JSON.stringify(message));
      const modalRef = this.modalService.open(MessageComponent);
