...
export class ShowFilmsComponent implements OnInit {
  ...
  initializeTopRated(num: number) {
    this.filmService.getTopRated(num).subscribe((films: Film[]) => {
      this.topRatedFilms = films;
    });
  }

  initializeMostPopulard(num: number) {
    this.filmService.getMostPopular(num).subscribe((films: Film[]) => {
  ...
  initializeRecommended(num: number) {
    this.filmService
      .filmsRecommended(num)
      .subscribe((films: RecommendedFilm[]) => {
        this.recommendedFilms = films;
      });
  }

  initializeArtists() {
    this.artistService.getArtists().subscribe((allArtists: Artist[]) => {
  ...
  showTopRated() {
    this.filmService.getTopRated(100).subscribe((films: Film[]) => {
      this.films = films;
    });
    this.searched = true;
  }
  ...
  showAllMostPopular() {
    this.filmService.getMostPopular(100).subscribe((films: Film[]) => {
      this.films = films;
    });
    this.searched = true;
  }
  ...
  showAllRecommended() {
    if (
      this.loggedIn == true &&
      this.loggedUser.role === "ROLE_REGISTERED_USER"
    ) {
      this.filmService
        .filmsRecommended(100)
        .subscribe((films: RecommendedFilm[]) => {
  ...
  showSearchedByName() {
    this.filmService.getByName(this.searchedFilm).subscribe((films: Film[]) => {
      this.films = films;
    });
    this.searched = true;
  }
  ...
  showSearchedByGenre() {
    if (
      ...
      this.filmService
        .filmsRecommendedByGenre(100, this.searchedGenre)
        .subscribe((films: RecommendedFilm[]) => {
          let recommendedFilms: Film[] = [];
          films.forEach((element) => {
            recommendedFilms.push(element.film);
      ...
      this.filmService
        .filmsSearchdByGenre(this.searchedGenre)
  ...
  showSearchedByArtist(id: string) {
    if (
      ...
      this.films = [];
      this.filmService
        .filmsRecommendedByArtist(100, id)
        .subscribe((recommended: RecommendedFilm[]) => {
          recommended.forEach((element) => {
            this.films.push(element.film);
          });
        });
    } else {
      this.filmService.filmsSearchByArtist(id).subscribe((films: Film[]) => {
        this.films = films;
      });
    }
    this.searched = true;
