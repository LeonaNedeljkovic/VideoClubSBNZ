...
export class AgeTemplateComponent implements OnInit {
  ...
  initAgeClassifiers() {
    this.ageClassifierService.getAll().subscribe((response) => {
  ...
  saveAgeClassifiers() {
    let classifiers = [
      this.childClassifier,
      this.teenClassifier,
      this.youngAdultClassifier,
      this.adultClassifier,
      this.elderClassifier,
    ];
    this.ruleService.classifyUserByAge(classifiers).subscribe((response) => {
