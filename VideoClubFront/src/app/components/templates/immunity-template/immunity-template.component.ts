...
export class ImmunityTemplateComponent implements OnInit {
  ...
  initializeTitleInfo(){
    this.ruleService.getImmunityInfo().subscribe(
  ...
  setBronzeAcquire(){
    ...
    this.ruleService.setBronzeImmunityPoints(points).subscribe(
      (response) => {
        if(!response.value){
  ...
  setSilverAcquire(){
    ...
    this.ruleService.setSilverImmunityPoints(points).subscribe(
      (response) => {
        if(!response.value){
          this.errorMessage = response.result;
          this.greenMessage = "";
  ...
  setGoldAcquire(){
    ...
    this.ruleService.setGoldImmunityPoints(points).subscribe(
      (response) => {
        if(!response.value){
          this.errorMessage = response.result;
          this.greenMessage = "";
