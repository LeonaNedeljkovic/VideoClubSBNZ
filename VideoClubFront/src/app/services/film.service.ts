...
export class FilmService {
  ...
  filmsRecommended = (num: number): Observable<RecommendedFilm[]> => {
    return this.http
      .get<RecommendedFilm[]>(`/api/films/recommended/${num}`)
      .pipe(
        map((res: any) => {
          return res;
        })
  ...
  filmsRecommendedByArtist = (
    num: number,
    id: string
  ): Observable<RecommendedFilm[]> => {
    return this.http
      .get<RecommendedFilm[]>(`/api/films/recommended/artist/${num}/${id}`)
      .pipe(
        map((res: any) => {
          return res;
        })
  ...
  filmsRecommendedByGenre = (
    num: number,
    genre: string
  ): Observable<RecommendedFilm[]> => {
    return this.http
      .get<RecommendedFilm[]>(`/api/films/recommended/genre/${num}/${genre}`)
      .pipe(
        map((res: any) => {
          return res;
        })
  ...
  filmsSearchByArtist = (id: string): Observable<Film[]> => {
    return this.http.get<Film[]>(`/api/films/search/artist/${id}`).pipe(
      map((res: any) => {
        return res;
      })
    );
  ...
  filmsSearchdByGenre = (genre: string): Observable<Film[]> => {
